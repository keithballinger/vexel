...
nohup go run -tags metal ./inference/cmd/vexel \
  ...
  train --data experiments/domain-jargon/train_augmented.jsonl \
  --output experiments/domain-jargon/adapter_augmented \
  --rank 16 --alpha 32 --lr 5e-5 --momentum 0.9 --epochs 20 \
  > experiments/domain-jargon/training_augmented.log 2>&1 &
echo "PID: $!"
